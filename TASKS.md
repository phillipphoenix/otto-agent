...
- [ ] Modify `src/commands/run.ts` to start the update check in the background before rendering and show `UpdateNotice` via a second Ink pass after the main app exits — file(s): `src/commands/run.ts`
